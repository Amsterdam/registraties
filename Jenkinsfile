...
node {
    stage("Build acceptance image") {
        tryStep "build", {
            def image = docker.build("build.app.amsterdam.nl:5000/ois/wonen:${env.BUILD_NUMBER}",
                "--shm-size 1G " +
    ...
    node {
        stage('Push acceptance image') {
            tryStep "image tagging", {
                def image = docker.image("build.app.amsterdam.nl:5000/ois/wonen:${env.BUILD_NUMBER}")
                image.pull()
        ...
        stage("Deploy to ACC") {
            tryStep "deployment", {
                ...
                parameters: [
                    ...
                    [$class: 'StringParameterValue', name: 'PLAYBOOK', value: 'wonen.yml'],
                ]
            }
        }
    }
